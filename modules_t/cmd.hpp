...
public:
    ...
    static Cmd& parse_args(int argc, char* argv[])
    {
        ...
        for (size_t i = 1; i < args.size(); ++i) 
        {
            ...
            switch (state) 
            {
                ...
                {
                    cmd._class_path = arg;
                    if(cmd._class_path.back() == '\\' || cmd._class_path.back() == '/') {
                        cmd._class_path.pop_back(); // Remove trailing backslash or slash
                    }
                ...
                {
                    cmd._Xjre_path = arg;
                    if(cmd._Xjre_path.back() == '\\' || cmd._Xjre_path.back() == '/') {
                        cmd._Xjre_path.pop_back(); // Remove trailing backslash or slash
                    }
    ...
    bool is_success() const { return _parse_sucess; }
    bool is_help() const { return _help_flag; }
    bool is_version() const { return _version_flag; }
    const std::string& get_error_msg() const { return _error_msg; }
    const std::string& get_jre_path() const { return _Xjre_path; }
    const std::string& get_class_path() const { return _class_path; }
    const std::string& get_java_class() const { return _java_class; }
...
private:
    ...
    std::string _pname; // 程序名
    
    std::string _cp_option; // -cp or -classpath
    std::string _Xjre_option; // -Xjre
